...
public class RecommendActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btn_text1.setOnClickListener(v -> {
            ...
            imageRequester.requestImage("false", btn_text1.getText().toString(), "Picasso", "100");
        });

        btn_text2.setOnClickListener(v -> {
            loading();
            ImageRequester imageRequester = new ImageRequester(serverIp, serverPort, imageCallback);
            imageRequester.requestImage("false", btn_text2.getText().toString(), "Picasso", "100");
        });

        btn_text3.setOnClickListener(v -> {
            loading();
            ImageRequester imageRequester = new ImageRequester(serverIp, serverPort, imageCallback);
            imageRequester.requestImage("false", btn_text3.getText().toString(), "Picasso", "100");
        });

        btn_text4.setOnClickListener(v -> {
            loading();
            ImageRequester imageRequester = new ImageRequester(serverIp, serverPort, imageCallback);
            imageRequester.requestImage("false", btn_text4.getText().toString(), "Picasso", "100");
        });
